%% Stop Infusion Pump
function  Char_Pump_CommandStop()
%% Declare string and key field that is equal to infusion pump driver
% Initializes values the sends to Pump
% Written  out for users readability!!!!
...
MatlabInterface.value(1,16) = uint8('S')
MatlabInterface.value(1,17) = uint8('T')
MatlabInterface.value(1,18) = uint8('O')
MatlabInterface.value(1,19) = uint8('P')
MatlabInterface.value(1,20) = 13;
MatlabInterface.value(1,21) = 13;

%% Write Value and key to the infusion pump
assignin('base','MatlabInterface',MatlabInterface);
dp = evalin('base','dp');                                                  %assigning in the Domain participant for the infusion pump
